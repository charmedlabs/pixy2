...
int Chirp::assemble(uint8_t type, ...)
{
	...
	{
		if (!m_hinformer)
			return CRP_RES_ERROR;  // don't send xdata if no hinformer
        m_call = false;
	}
...
int Chirp::sendChirpRetry(uint8_t type, ChirpProc proc)
{
    ...
    if (m_link==NULL || (!m_connected && !(type&CRP_INTRINSIC)))
        return CRP_RES_ERROR_NOT_CONNECTED;
	
	if (type==CRP_XDATA && !m_hinformer) // don't send xdata if no hinformer
