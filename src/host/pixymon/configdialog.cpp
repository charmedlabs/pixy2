...
ConfigDialog::ConfigDialog(QWidget *parent, Interpreter *interpreter) : QDialog(parent), m_ui(new Ui::ConfigDialog)
{
    ...
    // get rid of context help button
    ...
    // setup video widget dialog
    m_interpreter->m_video->setupDialog(m_pixymonTabs);
    ...
    for (int i=0; i<parameters.size(); i++)
    {
        ...
        if (type==PT_FLT32)
        {
            ...
            if (flags&PRM_FLAG_SLIDER)
            {
                ...
                slider->blockSignals(true); // block signals so we don't go right into handleSlider()
...
{
    // if we've already created this, remove and delete
    if (m_pixyTabs)
    {
        m_ui->pixyLayout->removeWidget(m_pixyTabs);
        delete m_pixyTabs;
        m_pixyTabs = NULL;
    }
    ...
    m_interpreter->m_pixyParameters.sort();
        ...
        if (type!=PT_INTS8) // make sure it's a scalar type
        {
            ...
            else if (flags&PRM_FLAG_SLIDER)
            {
                ...
                else
                {
                    if (flags&PRM_FLAG_SIGNED)
                    {
                        line->setText(QString::number(parameter.valueInt()));
                        pos = (parameter.valueInt() - min)*SLIDER_SIZE/(max - min);
                    }
                    else
                    {
                        line->setText(QString::number(parameter.value().toUInt()));
                        pos = (parameter.value().toUInt() - min)*SLIDER_SIZE/(max - min);
                    }
                }
                // value = min + pos/100(max-min)
                //(value - min)100/(max-min) = pos
...
void ConfigDialog::reject()
{
    ...
    // calling saveParams will save only shadowed parameters (we didn't call updatedb)
    // saving the parameters will cause the dirty bit to be set on pixy and all parameters will
...
void ConfigDialog::handleSlider()
{
