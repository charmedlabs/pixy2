...
void ConsoleWidget::print(QString text, uint flags)
{
    uint cflags;
    QColor color = CW_DEFAULT_COLOR;
    if (text=="")
        return;
    cflags = flags&TEXT_FLAG_COLOR_MASK;
    if (cflags)
    {
        if (cflags==TEXT_FLAG_BLUE)
            color = Qt::blue;
        else // if (cflags==TEXT_FLAG_RED)
            color = Qt::red;
    }
    m_histIndex = -1;
...
{
    print("error: " + text, TEXT_FLAG_RED);
}
