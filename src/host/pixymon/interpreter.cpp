        ...
        if (type==CRP_TYPE_HINT)
        {
            ...
            if (*(uint32_t *)args[0]==FOURCC('E','V','T','1'))
            {
                ...
                if (event==EVT_PARAM_CHANGE)
                    emit paramChange();
                else if (event==EVT_RENDER_FLUSH)
                    m_renderer->emitFlush();
                else if (event==EVT_PROG_CHANGE)
                {
                    queueCommand(GET_ACTIONS_VIEWS); // new program, update actions and views
                    emit paramChange();
                }
            }
            else if (*(uint32_t *)args[0]==FOURCC('T','E','X','T'))
            {
                flags = *(uint32_t *)args[1];
                flags |= TEXT_FLAG_BLUE;
                m_print +=  (char *)args[2];
            }
            else
            {
...
void Interpreter::getRunning()
{
    ...
    res = m_chirp->callSync(m_exec_running, END_OUT_ARGS, &running, &status, END_IN_ARGS);
    ...
    else if (m_running!=running || m_status!=status)
    {
        if (running==2)
            m_fastPoll = true;
        else
        ...
        m_running = running;
        m_status = status;
        emit runState(m_running, m_status);
        emit enableConsole(!m_running);
    }
}
...
int Interpreter::sendGetAction(int index)
{
    ...
    {
        if (index==0)
            emit actionScriptlet("", QStringList(""), true); // trigger the action list to clear if there are no actions
        return response;
    }

    scriptlet2 = parseScriptlet(scriptlet);

    emit actionScriptlet(action, scriptlet2, index==0);
    return response;
}

int Interpreter::sendGetView(int index)
{
    int res, response;
    char *viewName;
    res = m_chirp->callSync(m_exec_get_view, UINT16(index), END_OUT_ARGS, &response, &viewName, END_IN_ARGS);
    if (res<0)
        return res;
    ...
    emit view(viewName, index, response, (index==0));
    return response;
}

int Interpreter::sendSetView(uint index)
{
    int res, response;
    res = m_chirp->callSync(m_exec_set_view, UINT16(index), END_OUT_ARGS, &response, END_IN_ARGS);
    if (res<0)
        return res;
...
int Interpreter::sendGetProg(uint index)
{
    int res, response;
    char *progName, *progDesc;

    res = m_chirp->callSync(m_exec_get_prog, UINT16(index), END_OUT_ARGS, &response, &progName, &progDesc, END_IN_ARGS);
    if (res<0)
        return res;
    ...
    emit prog(progName, progDesc, index, index==0);
    return response;
}
...
void Interpreter::handlePendingCommand()
{
    ...
    case LOAD_PARAMS:
        handleLoadParams(command.m_arg0.toBool());
        break;
    ...
    case GET_ACTIONS_VIEWS:
        getActionsViews();
        break;

    case SET_VIEW:
        sendSetView(command.m_arg0.toUInt());
        break;
...
{
    return m_video->saveImage(filename);
}

void Interpreter::getActionsViews()
{
    uint i;
    // get all actions
    ...
    // get all views
    for (i=0; sendGetView(i)>=0; i++);
}

void Interpreter::getProgs()
{
    uint i;
    for (i=0; sendGetProg(i)>=0; i++);
}
...
#define PIXY_SIG_ALL                  0xff // all bits or'ed together
#define PIXY_ARRAYSIZE                       21
        m_exec_run = m_chirp->getProc("ser_packet");
        buf[0] = PIXY_SIG_ALL;
        buf[1] = PIXY_ARRAYSIZE;

        while(1)
        {
            //res = m_chirp->callSync(m_exec_run, UINT8(PIXY_TYPE_REQUEST_BLOCKS), UINTS8(2, buf), END_OUT_ARGS, &responseInt, &dataType, &dataLen, &data, END_IN_ARGS);
            res = m_chirp->callSync(m_exec_run, UINT8(PIXY_TYPE_REQUEST_LINES), UINTS8(0, buf), END_OUT_ARGS, &responseInt, &dataType, &dataLen, &data, END_IN_ARGS);
            if (res<0)
                qDebug("error");
            else if (dataType==PIXY_TYPE_RESPONSE_LINES)
                qDebug("%d: %d %d %d", dataLen, data[0], data[1], data[2]);
        ...
        if (res<0)
            throw std::runtime_error("Can't get firmware version.");
        memcpy(m_version, ver, 6*sizeof(uint16_t));
        ...
        emit version(m_version[0], m_version[1], m_version[2], m_versionType, m_version[3], m_version[4], m_version[5]);
        ...
        m_exec_get_prog = m_chirp->getProc("getProg");
        m_exec_get_action = m_chirp->getProc("getAction");
        m_exec_get_view = m_chirp->getProc("getView");
        m_exec_set_view = m_chirp->getProc("setView");
        ...
        if (m_exec_run<0 || m_exec_running<0 || m_exec_stop<0 || m_exec_get_prog<0 || m_exec_get_action<0 ||
                m_exec_get_view<0 || m_exec_set_view<0 ||
...
void Interpreter::handleLoadParams(bool contextual)
{
    DBG("loading...");
    uint i;
    char *id, *desc;
    uint32_t len;
    uint32_t flags, priority;
    int response, res;
    ...
    for (i=0; true; i++)
    {
        ...
        res = m_chirp->callSync(m_getAll_param, UINT8(contextual), UINT16(i), END_OUT_ARGS, &response, &flags, &priority, &argList, &id, &desc, &len, &data, END_IN_ARGS);
        if (res<0)
            break;
        ...
        parameter.setProperty(PP_PRIORITY, priority);
