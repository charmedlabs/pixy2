...
const QString &Parameter::id() const
{
    return m_id;
}
...
QVariant Parameter::property(const QString &label) const
{
    for (int i=0; i<m_properties.size(); i++)
    {
        if (m_properties[i].first==label)
...
bool compParameter(const Parameter &a, const Parameter &b)
{
    uint prioritya=0, priorityb=0;
    QVariant va = a.property(PP_PRIORITY);
    QVariant vb = b.property(PP_PRIORITY);
    ...
    if (prioritya!=priorityb) // high priority goes first, so invert the comparision
