...
typedef uint32_t (*SerialTxCallback)(uint8_t *data, uint32_t len); 
...
{
public:
	TransmitQ(uint32_t size, SerialTxCallback callback)
	{
		m_size = size;
		m_buf = new BufType[m_size];
		m_callback = callback;
...
public:
	...
	virtual int update() // called roughly once per frame to do maintenance tasks
	{
		return 0;
	}
	virtual int startTransmit() // called to kick off transmit, e.g. UART
	{
		return 0;
	}
};
