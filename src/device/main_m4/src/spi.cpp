...
Spi::Spi(SerialTxCallback callback) : m_rq(SPI_RECEIVEBUF_SIZE), m_tq(SPI_TRANSMITBUF_SIZE, callback)
{
	uint32_t i;
	volatile uint32_t d;
	...
	// clear all status and interrupt enable bits 
	SSP_ClearIntPending(LPC_SSP1, SSP_IMSC_BITMASK);
	SSP_IntConfig(LPC_SSP1, SSP_IMSC_BITMASK, DISABLE);
	SSP_IntConfig(LPC_SSP1, SSP_INTCFG_RX, ENABLE);
...
void spi_init(SerialTxCallback callback)
{
