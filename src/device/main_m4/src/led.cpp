...
void led_init()
{
	...
	g_ledOnCurrent[LED_BLUE] = g_ledOnCurrent[LED_GREEN];
...
void led_setPWM(uint8_t led, uint16_t pwm)
{
	...
	if (pwm==LED_MAX_PWM)
	{
		LPC_SCT->OUT[channel].SET = 1<<3;
	 	LPC_SCT->OUT[channel].CLR = 1<<15; 
	}
	else
	{
		...
		LPC_SCT->OUT[channel].SET = 1<<3;
		LPC_SCT->OUT[channel].CLR = 1<<(led+4);
	}	
}
...
int32_t led_setLamp(const uint8_t &upper, const uint8_t &lower)
{
	...
	if (lower) // set LEDs to max
		pwm = LED_MAX_PWM;
	else 
		pwm = 0;
		
	led_setPWM(LED_RED, pwm);		
