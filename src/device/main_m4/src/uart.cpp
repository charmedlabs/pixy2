...
void Uart::irqHandler()
{
	...
	if (status==UART_IIR_INTID_RDA) // Receive Data Available 
	{
		...
		if (!g_oldProtocol)
			ser_rxCallback(); // new protocol receive callback
	}
...
Uart::Uart(LPC_USARTn_Type *uart,  SerialTxCallback callback) : m_rq(UART_RECEIVE_BUF_SIZE), m_tq(UART_TRANSMIT_BUF_SIZE, callback)
{
...
void uart_init(SerialTxCallback callback)
{
