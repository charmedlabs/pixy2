...
void CameraInit(void)
{
	...
	LPC_GPIO_PORT->DIR[0] = 0x000f;
	LPC_GPIO_PORT->DIR[1] &= ~0xfbff;

	// toggle camera reset
	LPC_GPIO_PORT->PIN[0] = 0;
	LPC_GPIO_PORT->PIN[0] &= ~(0x0008);
...
void GPIOInit(void)
{
	...
	// set up button IO
	LPC_SGPIO->GPIO_OENREG &= ~(1<<15); 

	// set up white LED IO
	LPC_GPIO_PORT->MASK[1] = 0;
	LPC_GPIO_PORT->DIR[1] = 1<<10;	
	LPC_GPIO_PORT->PIN[1] &= ~(1<<10); // turn off
}
...
#define AWB_TIMEOUT   7*1000*1000  // 7 seconds

void handleAWB()
{
	static uint32_t timer;
	static uint8_t state = 0;
	uint8_t awbp=0, awb=0;
...
#endif
{
 ...
 //	STACK_GUARD = STACK_GUARD_WORD;
	...
	g_chirpUsb = new Chirp(false, false); 
