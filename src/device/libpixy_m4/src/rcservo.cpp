...
void rcs_shadowCallback(const char *id, const uint16_t &val)
{
	rcs_setLimits();
}
...
void rcs_loadParams()
{
	...
	prm_add("S0 lower limit", PRM_FLAG_SIGNED | PRM_FLAG_SLIDER, PRM_PRIORITY_DEFAULT,
		"@c Servo @m -300 @M 300 Sets the lower limit of travel for servo 0 (default -200)", INT16(-200), END);
	prm_setShadowCallback("S0 lower limit", (ShadowCallback)rcs_shadowCallback);
	prm_add("S0 upper limit", PRM_FLAG_SIGNED | PRM_FLAG_SLIDER, PRM_PRIORITY_DEFAULT,
		"@c Servo @m -300 @M 300 Sets the upper limit of travel for servo 0 (default 200)", INT16(200), END);
	prm_setShadowCallback("S0 upper limit", (ShadowCallback)rcs_shadowCallback);
	prm_add("S1 lower limit", PRM_FLAG_SIGNED | PRM_FLAG_SLIDER, PRM_PRIORITY_DEFAULT,
		"@c Servo @m -300 @M 300 Sets the lower limit of travel for servo 1 (default -200)", INT16(-200), END);
	prm_setShadowCallback("S1 lower limit", (ShadowCallback)rcs_shadowCallback);
	prm_add("S1 upper limit", PRM_FLAG_SIGNED | PRM_FLAG_SLIDER, PRM_PRIORITY_DEFAULT,
		"@c Servo @m -300 @M 300 Sets the upper limit of travel for servo 1 (default 200)", INT16(200), END);
	prm_setShadowCallback("S1 upper limit", (ShadowCallback)rcs_shadowCallback);
	
	prm_add("Servo frequency", PRM_FLAG_ADVANCED, PRM_PRIORITY_DEFAULT, 
...
int32_t rcs_setPos(const uint8_t &channel, const uint16_t &pos)
{
	...
	LPC_SCT->OUT[c].SET = 1<<0; 
	LPC_SCT->OUT[c].CLR = 1<<(channel+1);
...
int32_t rcs_enable(const uint8_t &channel, const uint8_t &enable)
{
	...
	{
		LPC_SCT->OUT[c].SET = 1<<15; // disable
		LPC_SCT->OUT[c].CLR = 1<<0;
	}
