...
struct SmMap
{
	...
	volatile uint8_t streamState;
	volatile uint16_t currentLine;
	volatile uint16_t frameTime;
	volatile uint16_t blankTime;
	volatile uint16_t recvStatus; // NOTE, M0 recv and send flags are backwards
